...
export class WarrantyService {
    public async processWarrantyClaim(
        orderID: number,
        userID: string,
    ): Promise<void> {
        ...
        try {
            ...
            // Log success to Storage Queue
            await storageQueueIntegration.sendMessage({
                status: "success",
                message: "Warranty claim processed and sent to Service Bus",
                orderID,
                userID,
            ...
            console.error(
                `Error processing warranty claim for Order ${orderID}:`,
                error,
            );

            // Log error to Storage Queue
            await storageQueueIntegration.sendMessage({
                status: "error",
